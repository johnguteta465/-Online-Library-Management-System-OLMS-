...
if (isset($_POST['submit'])) {
    ...
    $is_available_online = isset($_POST['is_available_online']) ? 1 : 0; // NEW FIELD

    if (empty($title) || empty($author) || empty($isbn) || $quantity <= 0) {
        $message = "Please fill in all required fields (Title, Author, ISBN, Quantity must be greater than 0).";
        $message_type = 'error';
    } else {
        ...
        if ($message_type !== 'error') {
            ...
            // UPDATED SQL QUERY: Added is_available_online field
            $stmt = $conn->prepare("INSERT INTO books (title, author, publisher, category, year, isbn, quantity, cover, pdf, is_available_online, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            
            if (!$stmt) {
                $message = "Database prepare failed: (" . $conn->errno . ") " . $conn->error;
                $message_type = 'error';
            } else {
                // UPDATED BIND PARAM: Added is_available_online parameter
                $stmt->bind_param("ssssisiisss", $title, $author, $publisher, $category, $year, $isbn, $quantity, $coverPath, $pdfPath, $is_available_online, $created_at);

                if ($stmt->execute()) {
                    $message = "Book '**" . htmlspecialchars($title) . "**' added successfully with quantity: $quantity!";
...
<title>Add Book - Admin | Ambo University Library</title>
...
<div class="dashboard">
    ...
    <form action="add_book.php" method="POST" enctype="multipart/form-data">
        <input type="text" name="title" placeholder="Book Title *" required value="<?= htmlspecialchars($_POST['title'] ?? '') ?>">
        <input type="text" name="author" placeholder="Author *" required value="<?= htmlspecialchars($_POST['author'] ?? '') ?>">
        
        <input type="text" name="publisher" placeholder="Publisher" value="<?= htmlspecialchars($_POST['publisher'] ?? '') ?>">
        <input type="text" name="category" placeholder="Category (e.g., Science, Fiction)" value="<?= htmlspecialchars($_POST['category'] ?? '') ?>">
        
        <input type="number" name="year" placeholder="Publication Year (e.g., 2023)" required value="<?= htmlspecialchars($_POST['year'] ?? '') ?>" min="1000" max="2024">
        <input type="text" name="isbn" placeholder="ISBN *" required value="<?= htmlspecialchars($_POST['isbn'] ?? '') ?>">
        
        <div class="full-width">
            <input type="number" name="quantity" placeholder="Quantity in Stock *" required value="<?= htmlspecialchars($_POST['quantity'] ?? '') ?>" min="1" max="999">
        </div>
        
        <!-- NEW FIELD: Available for Online Reading -->
        <div class="file-upload-group">
            <label>Available for Online Reading:</label>
            <select name="is_available_online" required>
                <option value="1" selected>Yes - Digital Version Available</option>
                <option value="0">No - Physical Copy Only</option>
            </select>
        </div>
